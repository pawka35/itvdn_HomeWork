...
function levelUp(){//функция изменения этапа (подниятие скорости)

};

function addScore() { //функция прибавления набранных очков
  console.log(scoreDiv.innerHTML);
  scoreDiv.innerHTML = parseInt(scoreDiv.innerHTML) + addScoreVal;
  if (score%10 == 0){//каждые 10 очков повышаем скорость, увеличиваем очки
    if (speed>50){//чтобы не уйти уж совсем в малые значения
      speed-=20;
    }
...
function zm(x, y) { //конструктор для задания координат центра для окружности на холсте
  this.x = x;
  this.y = y;
  this.lastX = x;
  this.lastY = y;
...
function createCandy(colvo) { //функция для создания конфет (кол-во создаваемых примает в параметре)
  for (i = 0; i < colvo; i++) {
    candy.push(
      new zm(
...
function gameOver() { // функция остановки игру и показа окна "вы проиграли"
  clearInterval(timer);
...
(function prestart() { //инициализация игры
  for (ball of body) { //рисуем головову змеи
    context.beginPath();
  ...
  for (ball of candy) { ///рисуем ранее созданные конфеты
    context.beginPath();
    context.arc(ball.x, ball.y, radius, 0, Math.PI * 2, true);
...
function checkColision() { //функция проверки столкновения "головы змей"
  let head = body[0]; //берем в отдельную переменную голову
  if ( //проверка выхода головы за пределы игрового поля (канваса)
  ...
  for (i = 1; i < body.length; i++) { //проверка столкновения головы с любой составляющей тела
  ...
  for (can of candy) { //проверка на столкновение головы с "конфетой"
    let f = Math.sqrt((can.x - head.x) ** 2 + (can.y - head.y) ** 2); //расстояние между центрами окружностей
    if (f < radius*2) { //если на расстоянии меньше, чем диамето
...
function DrawBall() { //функция отрисовки игры (вызывается в таймере)
  context.clearRect(0, 0, canvas.width, canvas.height); //чистим поле
  for (let i = 0; i < body.length; i++) { //для каждого звена змейки
    ...
    context.beginPath();
    if (i == 0) { // если рисуем голову змеи
      context.arc( // рисуем новый круг, учитывая шаг смещения
        (body[i].x += dx),
        (body[i].y += dy),
        radius,
        0,
      ...
      checkColision();
    } else {//если отрисовываем другие части змеи
  ...
  for (ball of candy) {//рисуем конфетки
    context.beginPath();
    context.arc(ball.x, ball.y, radius, 0, Math.PI * 2, true);
    context.fillStyle = "yellow";
    context.fill();
...
document.onkeydown = function(e) {//следим за тем, что нажал пользователь
  e.preventDefault(); //чтобы не прокручивался экран, при нажатии на стрелки
  // console.log(e.keyCode);
  switch (e.keyCode) {//в зависимости от нажатой клавиши, меняем величину и знак приращения центра координат
    case 40: {
      if (dy == -30) { //если змея шла вниз, нельзя ее сразу направить вверх
        break;
      } else {
        dx = 0;
        dy = 30;
    ...
    case 32:{ //нажатие пробела
      console.log(timer);
      if (timer==null){//если игра не запушена, запускаем, иначе останавливаем (ВАШ К.О)
        if (dx==0 && dy==0){dx=30};//если не куда не двигались, начинаем движение вправо
        timer = setInterval(DrawBall,speed);
      }else{
        clearInterval(timer);
        timer = null;
...
function randomize(min, max) {//функиця генерации случайного значения из диапазона
